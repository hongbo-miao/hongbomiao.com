...
module "snowflake_production_department_db_schema_read_only_role" {
  ...
  snowflake_role_name = "HM_PRODUCTION_${each.value.department_name}_DB_${each.value.schema_name}_READ_ONLY_ROLE"
}
...
module "snowflake_production_department_db_schema_read_write_role" {
  ...
  source              = "../../../../modules/snowflake/hm_snowflake_role"
  for_each            = { for x in local.production_department_db_department_name_schema_name_list : "${x.department_name}.${x.schema_name}" => x }
  snowflake_role_name = "HM_PRODUCTION_${each.value.department_name}_DB_${each.value.schema_name}_READ_WRITE_ROLE"
}
...
module "grant_production_department_db_schema_read_only_role_to_production_department_db_schema_read_write_role" {
  ...
  for_each                     = { for x in local.production_department_db_department_name_schema_name_list : "${x.department_name}.${x.schema_name}" => x }
  snowflake_role_name          = "HM_PRODUCTION_${each.value.department_name}_DB_${each.value.schema_name}_READ_ONLY_ROLE"
  snowflake_grant_to_role_name = "HM_PRODUCTION_${each.value.department_name}_DB_${each.value.schema_name}_READ_WRITE_ROLE"
  depends_on = [
    module.snowflake_production_department_db_schema_read_only_role,
    module.snowflake_production_department_db_schema_read_write_role
  ]
...
module "snowflake_production_department_db_admin_role" {
  ...
  snowflake_role_name = "HM_PRODUCTION_${each.value}_DB_ADMIN_ROLE"
}
...
module "grant_production_department_db_schema_read_write_role_to_production_department_db_admin_role" {
  ...
  for_each                     = { for x in local.production_department_db_department_name_schema_name_list : "${x.department_name}.${x.schema_name}" => x }
  snowflake_role_name          = "HM_PRODUCTION_${each.value.department_name}_DB_${each.value.schema_name}_READ_WRITE_ROLE"
  snowflake_grant_to_role_name = "HM_PRODUCTION_${each.value.department_name}_DB_ADMIN_ROLE"
  depends_on = [
    module.snowflake_production_department_db_schema_read_write_role,
    module.snowflake_production_department_db_admin_role
  ]
...
module "grant_production_department_db_schema_admin_role_to_production_department_db_schema_admin_user" {
  ...
  snowflake_role_name = "HM_PRODUCTION_${each.value.department_name}_DB_ADMIN_ROLE"
...
locals {
  ...
  production_hm_streamlit_db_department_name_creator_name_list = flatten([
    for department in var.production_hm_streamlit_db_departments :
    [
  ...
  production_hm_streamlit_db_department_name_user_name_list = flatten([
    for department in var.production_hm_streamlit_db_departments :
    [
...
module "snowflake_production_hm_streamlit_db_department_creator_role" {
  providers           = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source              = "../../../../modules/snowflake/hm_snowflake_role"
  for_each            = local.production_hm_streamlit_db_department_names
  snowflake_role_name = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_CREATOR_ROLE"
}
module "grant_database_privileges_to_production_hm_streamlit_db_creator_role" {
  providers               = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                  = "../../../../modules/snowflake/hm_snowflake_grant_database_privileges_to_role"
  for_each                = local.production_hm_streamlit_db_department_names
  snowflake_role_name     = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_CREATOR_ROLE"
...
module "grant_schema_privileges_to_production_hm_streamlit_db_creator_role" {
  providers               = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                  = "../../../../modules/snowflake/hm_snowflake_grant_schema_privileges_to_role"
  for_each                = local.production_hm_streamlit_db_department_names
  snowflake_role_name     = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_CREATOR_ROLE"
...
module "grant_production_hm_streamlit_db_schema_creator_role_to_user" {
  providers           = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source              = "../../../../modules/snowflake/hm_snowflake_grant_role_to_user"
  for_each            = { for x in local.production_hm_streamlit_db_department_name_creator_name_list : "${x.department_name}.${x.creator_name}" => x }
  snowflake_role_name = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value.department_name}_CREATOR_ROLE"
  snowflake_user_name = each.value.creator_name
  depends_on = [
    module.snowflake_production_hm_streamlit_db_department_creator_role
  ]
}
module "grant_warehouse_privileges_to_production_hm_streamlit_db_department_creator_role" {
  providers                = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                   = "../../../../modules/snowflake/hm_snowflake_grant_warehouse_privileges_to_role"
  for_each                 = local.production_hm_streamlit_db_department_names
  snowflake_role_name      = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_CREATOR_ROLE"
...
module "snowflake_production_hm_streamlit_db_department_user_role" {
  providers           = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source              = "../../../../modules/snowflake/hm_snowflake_role"
  for_each            = local.production_hm_streamlit_db_department_names
  snowflake_role_name = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_USER_ROLE"
}
module "grant_database_privileges_to_production_hm_streamlit_db_user_role" {
  providers               = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                  = "../../../../modules/snowflake/hm_snowflake_grant_database_privileges_to_role"
  for_each                = local.production_hm_streamlit_db_department_names
  snowflake_role_name     = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_USER_ROLE"
...
module "grant_schema_privileges_to_production_hm_streamlit_db_user_role" {
  providers               = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                  = "../../../../modules/snowflake/hm_snowflake_grant_schema_privileges_to_role"
  for_each                = local.production_hm_streamlit_db_department_names
  snowflake_role_name     = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_USER_ROLE"
...
module "grant_production_hm_streamlit_db_schema_user_role_to_user" {
  providers           = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source              = "../../../../modules/snowflake/hm_snowflake_grant_role_to_user"
  for_each            = { for x in local.production_hm_streamlit_db_department_name_user_name_list : "${x.department_name}.${x.user_name}" => x }
  snowflake_role_name = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value.department_name}_USER_ROLE"
  snowflake_user_name = each.value.user_name
  depends_on = [
    module.snowflake_production_hm_streamlit_db_department_user_role
  ]
}
module "grant_warehouse_privileges_to_production_hm_streamlit_db_department_user_role" {
  providers                = { snowflake = snowflake.hm_production_terraform_read_write_role }
  source                   = "../../../../modules/snowflake/hm_snowflake_grant_warehouse_privileges_to_role"
  for_each                 = local.production_hm_streamlit_db_department_names
  snowflake_role_name      = "HM_PRODUCTION_HM_STREAMLIT_DB_${each.value}_USER_ROLE"
