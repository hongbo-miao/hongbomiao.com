...
use crate::shared::fusion::utils::project_radar_to_camera::project_radar_to_camera;
...
pub fn visualize_camera_radar_fusion<P: AsRef<Path>>(
    ...
    let fusion_result = fuse_camera_radar(camera_detections, radar_detections, 100.0, &config);
    ...
    let legend_text = "Green Box = Fused Track (Camera + Radar) | Blue Box = Camera Only | Yellow Point = Radar Only";
    put_text(
        &mut visualization,
        legend_text,
        Point::new(10, 60),
    ...
    imshow("Camera-Radar Fusion", &visualization)?;
    let key = wait_key(10)?; // ms
    if key == 'q' as i32 {
