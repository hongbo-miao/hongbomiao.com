...
for hex_digit in {0..9} {a..f}; do
  ...
  query="create external table awsdatacatalog.${database_name}.${table_name} location 's3://hongbomiao-bucket/data/delta-tables/${source}_data/${table_name}' tblproperties ('table_type' = 'delta')"
