...
impl WebRtcVadProcessor {
    ...
    pub fn process_frame(
        ...
        let normalized_frame = Self::normalize_frame(app_config, sample_rate_hz, frame);
    ...
    fn normalize_frame(app_config: &AppConfig, sample_rate_hz: u32, frame: &[i16]) -> Vec<i16> {
        let samples_per_frame = (sample_rate_hz as u64
    ...
    fn handle_speech_transitions(
        ...
        {
            Self::start_speech_segment(app_config, sample_rate_hz, speech_state, frame_buffer);
            return None;
        }
        ...
        if speech_state.is_in_speech {
            ...
            if speech_state.consecutive_silence_count >= end_needed {
                return Self::end_speech_segment(app_config, sample_rate_hz, speech_state);
            }
        }
    ...
    fn start_speech_segment(
        ...
        Self::add_padding_to_speech(app_config, sample_rate_hz, speech_state, frame_buffer);
    ...
    fn create_speech_segment(
        ...
        SpeechSegment {
            start_s: start_time,
            end_s: end_time,
            pcm_samples: speech_state.current_speech_samples.clone(),
        }
    }
        ...
        if speech_state.is_in_speech && !speech_state.current_speech_samples.is_empty() {
            ...
            if speech_duration_frames >= min_speech_frames {
                ...
                return Some(SpeechSegment {
                    start_s: start_time,
                    end_s: end_time,
                    pcm_samples: speech_state.current_speech_samples,
                });
            }
        }
        None
    ...
    pub fn samples_per_frame(app_config: &AppConfig, sample_rate_hz: u32) -> usize {
        (sample_rate_hz as u64 * app_config.webrtc_vad_frame_duration_ms as u64 / 1000) as usize
    }
}
